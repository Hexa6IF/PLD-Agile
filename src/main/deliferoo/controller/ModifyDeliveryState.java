...
public class ModifyDeliveryState implements State {
	...
	public void init(Window window, Controller controller) {
		Map<String, Map<String, BestPath>> tbp = new HashMap<String, Map<String, BestPath>>(
				controller.getCyclist().getBestPaths());
	...
	@Override
	public void changeNodePosition(Window window, Controller controller, Integer deliveryId, SpecialNodeType type,
			String newNodeId) {
		...
		Map<String, BestPath> fromTempPickup = Dijkstra.calculateShortestPaths(start, new HashSet<SpecialNode>(tr),
				controller.getCurrentMap());
		Map<String, BestPath> fromTempDropoff = Dijkstra.calculateShortestPaths(end, new HashSet<SpecialNode>(tr),
				controller.getCurrentMap());
	...
	public void confirmButtonClick(Window window, Controller controller) {
		...
		try {
			...
			Command modifyCommand = new CmdModifyDelivery(controller.getCyclist().getDeliveries(),
					controller.getCyclist().getRound(), controller.getSelectedDelivery(), controller.getTempDelivery(),
					controller.getCyclist().getBestPaths(), newBestPaths);
