	...
	public void loadDeliveries(Window window, Controller controller, File deliveriesFile, FullMap map) {
		try {
			...
			if (deliveries.size() > 0) {
				...
				window.updateMessage(
						"The loaded XML file does not match the expected format. Please correct it or load an other file.");
				window.clearDeliveriesMarkers();
				window.updateDeliveries(new ArrayList<>());
				window.clearDeliveriesRound();
				controller.setCurrentState(controller.MAP_LOADED_STATE);
			...
			window.updateMessage("Syntax error in loaded XML file. Please correct it or load an other file.");
			window.clearDeliveriesMarkers();
			window.updateDeliveries(new ArrayList<>());
			window.clearDeliveriesRound();
			controller.setCurrentState(controller.MAP_LOADED_STATE);
