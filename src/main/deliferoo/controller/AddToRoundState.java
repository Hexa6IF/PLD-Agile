...
public class AddToRoundState implements State{
	...
	for(int i = 0; i < tr.size(); i++) {
		...
		controller.doCommand(new CmdAddDelivery(controller.getCyclist().getRound(), controller.getCyclist().getDeliveries(), i + 1, controller.getTempDelivery()));
		controller.setSelectedDelivery(controller.getCyclist().getDeliveries().get(controller.getCyclist().getDeliveries().size() - 1));
