...
public class MapView {
	...
	Double x1 = this.offsetY
		+ dimension * ((edge.getStart().getLongitude() - this.map.getMinLong()) / this.map.getRangeLongitude());
	Double y1 = this.offsetX
		+ dimension * ((edge.getStart().getLatitude() - this.map.getMinLat()) / this.map.getRangeLatitude());

	Double x2 = this.offsetY
		+ dimension * ((edge.getEnd().getLongitude() - this.map.getMinLong()) / this.map.getRangeLongitude());
	Double y2 = this.offsetX
		+ dimension * ((edge.getEnd().getLatitude() - this.map.getMinLat()) / this.map.getRangeLatitude());
	...
	for (Delivery delivery : deliveries) {
	    ...
	    Circle startMarker = this.createMarker(start, dimension, deliveryColor);
	    Circle endMarker = this.createMarker(end, dimension, deliveryColor);
	    this.mapView.getChildren().addAll(startMarker,endMarker);
	}
    }

    private Circle createMarker(SpecialNode node, Double dimension, Paint paint) {
